...
stud = cq.Workplane('front')
stud = stud.workplane(offset=pad_thick)
...
a = (cq.Workplane('front').box(4.5, 3, 2).translate([2.25, -1.5, pad_thick +1 ]))
b = (cq.Workplane('front').box(2, 6, 2).translate([-1, 0, pad_thick +1 ]))
c = (cq.Workplane('front').box(6.5, 6, 2).translate([1.25, 0, pad_thick +3 ]))
grove = a.union(b)
grove = grove.union(c)
...
coax_support_side = coax_support_side.translate([0,2,0])
