...
grove_left = grove_left.translate([0, 2,0])
groves = grove_left.union(grove_right).translate([0, 18,0])
...
plane = result.workplane(centerOption='ProjectedOrigin',origin=(pad_x/2, pad_y/2 +8, 0))
...
result_A = result.union(cq.Workplane('front').text("54L15-DK",6,0.4 , kind="bold").translate([pad_x/2,0, pad_thick]))

show_object(result_A)

# Export first design as file formats used by 3D printers
cq.exporters.export(result_A, "./step/54L15-DK_clip.step")
cq.exporters.export(result_A, "./stl/54L15-DK_clip.stl")
...
coax_support_side = coax_support_side.translate([0, 20,0])
...
# Add text for identification of supported DK's
result_Murata = result_Murata.union(cq.Workplane('front').text("54L15-Murata",6,0.4 , kind="bold").translate([pad_x/2,0, pad_thick]))
...
cq.exporters.export(result_Murata, "./step/54L15-DK_Murata_coax_clip.step")
cq.exporters.export(result_Murata, "./stl/54L15-DK_Murata_coax_clip.stl")
...
# Add text for identification of supported DK's
result_MS156 = result_MS156.union(cq.Workplane('front').text("54L15-MS156",6,0.4 , kind="bold").translate([pad_x/2,0, pad_thick]))
...
cq.exporters.export(result_MS156, "./step/54L15-DK-MS156_coax_clip.step")
cq.exporters.export(result_MS156, "./stl/54L15-DK-MS156_coax_clip.stl")
